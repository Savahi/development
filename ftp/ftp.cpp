...
static size_t readfunc(void *ptr, size_t size, size_t nmemb, FILE *stream)
{
	size_t nread;
	...
	char *server, char *user, char *password)
{
	...
	if (strlen(server) + strlen(user) + strlen(password) + lenDirectory + strlen(fileName) + 7 >= FTP_MAX_REMOTE_ADDR) {
		return -1;
	}
...
static int upload(CURL *curl, char *srcPath, char *remotePath)
{
	...
	if (pathLen - fileNameStart > FTP_MAX_REMOTE_ADDR) {
		return -2;
	}
...
{
	int status = ftpTestP(fileName, directory, _server, _user, _password, size);
	return status;
}


// Tests if a file exists at a server
int ftpTestP(char *fileName,					// A file name to test
	char *directory,							// A server directory to test in
	...
	unsigned long int *size)
{
	...
	long long remoteFileSizeByte = (long long)getRemoteFileSize(_remoteAddr);
...
int ftpUpload(char *srcFileName, char *dstFileName, char *dstDirectory) {
	int status = ftpUploadP(srcFileName, dstFileName, dstDirectory, _server, _user, _password);
	return status;
}
...
int ftpSetCredentials(char *server, char *user, char *password) {
	_ftpErrorCode = 0;
