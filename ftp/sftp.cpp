...
int sftpTest(char *fileName,		// A file name to test
	...
	long long *size)
{
	return sftpTestP(fileName, directory, _server, _user, _password, size);
}
...
int sftpUpload(char *srcFileName, char *dstFileName, char *dstDirectory) {
	return sftpUploadP(srcFileName, dstFileName, dstDirectory, _server, _user, _password);
}
